...
const IoTBindingConfigModal: React.FC<IoTBindingConfigModalProps> = ({
  ...
  const [selectedDataType, setSelectedDataType] = useState<IoTDataType | null>(null);
  ...
  const getSourceDataInfo = async (sourcePath: string) => {
    try {
      ...
      const mockSourceData = {
        ...
        'sensor.scale': { type: 'array', value: [2, 3, 4], length: 3, elementType: 'number' },
  ...
  useEffect(() => {
    if (visible) {
      ...
      if (editingBinding) {
        ...
        setSelectedDataType(editingBinding.dataType);
        setCurrentStep(0);
      } else {
        // 新增模式
  ...
  const handleConnectionChange = (connectionId: string) => {
    ...
    form.setFieldValue('sourceId', connectionId); 
  };
  ...
  const handleValidate = async () => {
    try {
      ...
      // 先尝试验证字段
      try {
        await form.validateFields(['protocol', 'dataType', 'sourceId']);
      } catch (error) {
        // 验证失败，但继续获取表单值
      ...
      console.log('📋 dataType:', values.dataType);
      console.log('📋 sourceId:', values.sourceId);
      console.log('📋 selectedProtocol:', selectedProtocol);
      console.log('📋 selectedConnection:', selectedConnection);
      console.log('📋 selectedDataType:', selectedDataType);
      
      // 验证必填字段（如果表单值为空，尝试从状态中获取）
      const missingFields = [];
      const protocol = values.protocol || selectedProtocol;
      const dataType = values.dataType || selectedDataType;
      const sourceId = values.sourceId || selectedConnection?.id;
      
      if (!protocol) missingFields.push('协议类型');
      if (!dataType) missingFields.push('数据类型');
      if (!sourceId) missingFields.push('连接配置');
      ...
      const binding: IoTBinding = {
        ...
        sourceId: sourceId || values.sourceId,
      };
      ...
      console.log('🔍 验证时fixedBinding对象详细信息:');
      console.log('  - protocol:', fixedBinding.protocol);
      console.log('  - dataType:', fixedBinding.dataType);
      console.log('  - sourceId:', fixedBinding.sourceId);
      console.log('  - 完整对象:', JSON.stringify(fixedBinding, null, 2));
  ...
  const handleTest = async () => {
    try {
      ...
      console.log('🔍 fixedBinding对象详细信息:');
      console.log('  - protocol:', fixedBinding.protocol);
      console.log('  - dataType:', fixedBinding.dataType);
      console.log('  - sourceId:', fixedBinding.sourceId);
      console.log('  - 完整对象:', JSON.stringify(fixedBinding, null, 2));
